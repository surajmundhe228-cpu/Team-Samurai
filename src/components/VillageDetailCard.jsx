...
function VillageDetailCard({
  ...
  useEffect(() => {
    ...
    fetch(
      `http://127.0.0.1:8000/api/xai-risk/${encodeURIComponent(
        village.village
      )}`
    )
      .then((res) => {
      ...
      .catch((err) => {
        console.warn(
          "Backend XAI fetch failed, using local calculation:",
          err
        );
        ...
        const fallbackFactors = [
          ...
          },
          {
            ...
            description:
              "Historical disaster frequency logs show repeated seasonal displacement patterns.",
          },
        ];

        const total = fallbackFactors.reduce(
        ...
        setXaiData({
          predicted_risk_score:
            village.risk_score || Number(total.toFixed(1)),

          xai_feature_contributions: fallbackFactors,

          model_metadata: {
            algorithm:
              "Weighted Multi-Factor Vulnerability Regressor",
          },
        });
      ...
      <div className="village-detail-card">
        ...
        >
          ...
          >
            <li>
              River Distance:{" "}
              <strong>
                {village.distance_from_river_km != null
            ...
            <li>
              Elevation:{" "}
              <strong>
                {village.elevation_m != null
                  ? `${village.elevation_m} m`
                  : "N/A"}
              </strong>
            </li>
            ...
            <li>
              Flood History:{" "}
              <strong>
                {village.flood_history || "Standard"}
              </strong>
            </li>
        ...
        >
          ...
          {loadingXai ? (
            ...
            >
              Analyzing feature factors...
            </p>
          ) : xaiData &&
            xaiData.xai_feature_contributions ? (
            <div
              style={{
                display: "flex",
            ...
            >
              {xaiData.xai_feature_contributions.map(
                (factor, idx) => (
                  <div
                    key={idx}
                    style={{
              ...
              >
                Engine:{" "}
                {xaiData.model_metadata?.algorithm ||
                  "Weighted Multi-Factor Vulnerability Regressor"}
              </span>
            </div>
          ) : (
            ...
            >
              No XAI metrics available.
            </p>
          )}
        </div>

        {/* =============================== */}
        {/* SAFE SHELTER ROUTE BUTTON */}
        {/* =============================== */}
        ...
        <button
          type="button"
          onClick={() => {
            if (typeof onHospitalRouteClick === "function") {
              onHospitalRouteClick();
            } else {
              console.error("Nearest hospital route handler is not connected.");
            }
          }}
          style={{
            width: "100%",
